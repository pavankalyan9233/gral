...
describe.sequential(`PageRank, Graph: ${graphName}`, () => {
  bench('GRAL', async () => {
    ...
    const kgsGraphId = await gral.loadGraph(jwt, gralEndpoint, graphName, [], [], [], 50);
    await gral.runPagerank(jwt, gralEndpoint, kgsGraphId, 10, 0.85);
    await gral.dropGraph(jwt, gralEndpoint, kgsGraphId);
...
describe.sequential(`WCC, Graph: ${graphName}`, () => {
  bench('GRAL', async () => {
    const jwt = await arangodb.getArangoJWT();
    const kgsGraphId = await gral.loadGraph(jwt, gralEndpoint, graphName, [], [], [], 50);
    await gral.runWCC(jwt, gralEndpoint, kgsGraphId, {});
    await gral.dropGraph(jwt, gralEndpoint, kgsGraphId);
  }, {iterations: ITERATIONS, warmupIterations: WARMUP_ITERATIONS});
...
describe.sequential(`SCC, Graph: ${graphName}`, () => {
  bench('GRAL', async () => {
    const jwt = await arangodb.getArangoJWT();
    const kgsGraphId = await gral.loadGraph(jwt, gralEndpoint, graphName, [], [], [], 50);
    await gral.runSCC(jwt, gralEndpoint, kgsGraphId, {});
    await gral.dropGraph(jwt, gralEndpoint, kgsGraphId);
  }, {iterations: ITERATIONS, warmupIterations: WARMUP_ITERATIONS});
...
describe.sequential(`Label Propagation, Graph: ${graphName}`, () => {
  bench('GRAL', async () => {
    const jwt = await arangodb.getArangoJWT();
    const kgsGraphId = await gral.loadGraph(jwt, gralEndpoint, graphName, [], [], ['_key'], 50);
    await gral.runCDLP(jwt, gralEndpoint, kgsGraphId, "_key");
